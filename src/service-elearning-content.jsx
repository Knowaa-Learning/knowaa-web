...
function ElearningContent() {
  ...
  return (
    <div className="ce">
      {/* 1, Opener */}
      <section className="ce__sec" ref={r1}>
        <div className="ce__inner">
          <h2 className="ce__h">Built for <span className="ce-stroke">Your Learners.</span><br />Not Learners in General.</h2>
          <p className="ce__p">
            Every KNOWAA experience is built from your learners&rsquo; reality, their role, their context, the specific moment they need to perform. The content is tailored to the people who will use it, not adapted from an existing framework but developed from their working environment up. Design, interaction, and visual language are all decisions made with a specific audience in mind, which is why the finished experience tends to feel familiar to the people inside it rather than generic. Learning that feels relevant gets retained. Retained learning changes how people work.
          </p>
        </div>
      </section>
      ...
      <section className="ce__sec ce__sec--grid">
        <div className="ce__inner ce__inner--wide">
          <div className="ce__grid">
            {[
              { name: 'Microlearning',           sub: 'Short, focused modules', icon: 'assets/icons/microlearning.svg', tint: '#EDE7F6' },
              { name: 'Gamification',            sub: 'Engagement by design', icon: 'assets/icons/gamification.svg', tint: '#FFE9DC' },
              { name: 'Adaptive learning paths', sub: 'Routed to the gap', icon: 'assets/icons/adaptive-paths.svg', tint: '#E3E1F7' },
              { name: 'AI powered learning',     sub: 'Personalized at scale', icon: 'assets/icons/ai-learning.svg', tint: '#E0F2EE' },
              { name: 'Learning experience design', sub: 'Studio-grade craft', icon: 'assets/icons/learning-experience.svg', tint: '#FCE4EC' },
              { name: 'Blended learning', sub: 'Practice in context', icon: 'assets/icons/scenario-based.svg', tint: '#E1ECFB' },
            ].map((c) => (
              <div className="ce__card" key={c.name}>
                <div className="ce__card-icon" aria-hidden style={{ background: c.tint || '#F5F4FB' }}>
                  {c.icon ? (
                    <img src={c.icon} alt="" className="ce__card-icon-img" />
                  ) : (
                    <svg viewBox="0 0 32 32" width="32" height="32">
                      <rect x="4" y="4" width="24" height="24" rx="6" fill="none" stroke="#0D0959" strokeWidth="1.5" strokeDasharray="3 3" opacity="0.4" />
                    </svg>
                  )}
                </div>
                <div className="ce__card-text">
                  <div className="ce__card-name">{c.name}</div>
                  <div className="ce__card-sub">{c.sub}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="ce__inner ce__inner--wide">
          <div className="ce__split">
            <div className="ce__split-media">
              <img src="assets/elearning-together.png?v=2" alt="Learners collaborating on a custom training module" className="ce__split-img" />
            </div>
            <div className="ce__split-rule" aria-hidden></div>
            <div className="ce__split-text">
              <h3 className="ce__sub">Built to change what people can do.</h3>
              <p className="ce__p">
                Every module we build is designed around a single question: what does this person need to be able to do that they cannot do right now? Not what they need to have watched. Not what box needs to be checked. Competency is the destination. The learning experience is the route. When creative craft and behavioral science share the same brief, the result is not just content your people finish. It is knowledge that stays, skills you can see, and capability your business can measure.
              </p>
            </div>
          </div>
        </div>
      ...
      <style>{`
        ...
        .ce__split {
          display: grid;
          grid-template-columns: minmax(280px, 1fr) 1px minmax(0, 1.4fr);
...
Object.assign(window, { ElearningContent });
