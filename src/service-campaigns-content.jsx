...
const WORDMARK_WORDS = ['Campaigns', 'Marketing', 'Communications', 'Nudging', 'Branding'];

function CampaignsWordmark() {
  const [active, setActive] = useStateCE(0);
  const [prev, setPrev] = useStateCE(null);
  useEffectCE(() => {
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    const id = setInterval(() => {
      setActive((i) => {
        setPrev(i);
        return (i + 1) % WORDMARK_WORDS.length;
      });
    }, 2200);
  ...
  return (
    <div className="kw-wordmark" aria-label="Internal Campaigns, Marketing, Communications, Nudging, Branding">
      ...
      <span className="kw-wordmark__sr">{WORDMARK_WORDS[active]}</span>
    </div>
  );
}
    ...
    <div className="ce">
      ...
      <section className="ce__sec ce__sec--grid">
        <div className="ce__inner ce__inner--wide">
          <CampaignsWordmark />
          <div className="ce__aphorism-block">
            <p className="ce__aphorism">Learning does not become memorable because it exists.</p>
            <p className="ce__aphorism ce__aphorism--em">It becomes memorable because people cannot ignore it.</p>
          </div>
        </div>
      </section>
      ...
      <style>{`
        ...
        .ce__aphorism + .ce__aphorism { margin-top: 4px; }
        .ce__aphorism--em {
          font-weight: 700;
          color: #3E33BB;
        }
        ...
        /* Leaving word is absolute overlay so the slot can resize cleanly */
        .kw-wordmark__word--out {
          position: absolute;
          left: 0; top: 0;
          animation: kwWordOut 480ms cubic-bezier(0.4, 0, 0.2, 1) both;
          pointer-events: none;
        }
        @keyframes kwWordIn {
        ...
        @keyframes kwWordOut {
          ...
          100% { opacity: 0; transform: translateY(-60%); filter: blur(8px); }
        }
        .kw-wordmark__sr {
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .kw-wordmark__word--out { opacity: 0; }
        }
        @media (max-width: 600px) {
          .kw-wordmark { font-size: clamp(20px, 5.6vw, 28px); padding: 0 20px; }
        }
...
Object.assign(window, { CampaignsContent });
